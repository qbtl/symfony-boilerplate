...
export class TimeAgo extends HTMLElement {
  ...
  updateText(date) {
    ...
    if (seconds >= 0) {
      this.innerHTML = `${prefix || "Il y a"} ${term.text.replace(
        "%d",
        Math.round(seconds / term.divide)
      )}`;
    } else {
      this.innerHTML = `${prefix || "Dans"} ${term.text.replace(
        "%d",
        Math.round(Math.abs(seconds) / term.divide)
      )}`;
    }
